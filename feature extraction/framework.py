...
class BoardExperiment:
    def __init__(self, path, label, conf:USARTConfig):
...
#        print("{}\n\n{}".format(self.config, self))
    ...
    def process_data(self, frame):
        ...
        self.data = pd.DataFrame(columns=['Time', 'Channel A'])
        self.data.loc[:,'Time'] = x
        self.data.loc[:,'Channel A'] = y
    ...
    def analyze_waveform(self, ss_no=1, display=False):
        ...
        transition = True
        for i, s in enumerate(self.data['Channel A']):
            if i == 0:
                continue
        ...
        if metrics_zeros == -1 or metrics_ones == -1:
            return -1
        #metrics_half_zeros = Metrics(np.array([self.config.device_voltage/2 for i in self.tmp['zeros']]), self.tmp['zeros']).evaluate(metrics=['cos'])
        #metrics_half_ones = Metrics(np.array([self.config.device_voltage/2 for i in self.tmp['ones']]), self.tmp['ones']).evaluate(metrics=['cos'])
...
#        sample['ave_tf'] = np.mean(self.tmp['tf'])
    ...
    def create_dataset(self):
        ...
        for i in range(1, len(self.ss)+1):
            ...
            if sample == -1:
                print("Error with waveform: " + '{}/{}'.format(self.exp_path, self.ss[i-1]) + " skipping...")
                continue
            
            for key, value in sample.items():
                if np.isnan(value):
                    print("Empty cell detected in waveform: " + '{}/{}'.format(self.exp_path, self.ss[i-1]) + " skipping...")
                    skip = True
                    break
    ...
    def print_waveform(self):
        ...
        ax.plot(self.data['Time'], self.data['Channel A'], color='k', linewidth=1)
        ax.set_xlabel("Time (ms)")
        ax.set_ylabel("Channel A")
