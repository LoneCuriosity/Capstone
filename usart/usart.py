...
def upload_arduino_code(code, com_port, baud_rate, fqbn):
    #arduino:avr:mega
    #arduino:sam:arduino_due_x
    
    try:
        print(f"Attempting to upload code to Arduino on {com_port} at {baud_rate} baud.")

        temp_dir = "temp_sketch"
        
        os.makedirs(os.path.join("usart" ,temp_dir), exist_ok=True)

        sketch_file = os.path.join(os.path.join("usart" ,temp_dir), f"{temp_dir}.ino")
        with open(sketch_file, "w") as file:
            file.write(code)

        cmd = [
            "arduino-cli", "compile", "--fqbn", fqbn,
            "--port", com_port, os.path.join("usart" ,temp_dir)
        ]
        try:
            ...
            if err.returncode == 1:
                ...
                shutil.rmtree(os.path.join("usart" ,temp_dir))
                return -1
...
def auto_capture_waveforms(device, folder_name, subfolder_name, board_name, board_letter, message, run_time_ms=1000):
    ...
    while waveform_count < 64:
        ...
        if is_waveform_in_center(v_values, IDLE_VOLTAGE, 20):
...
def usart_record_data(isArduino=False, fqbn="arduino:avr:mega"):
    ...
    for baud_rate in baud_rates:
        folder_name = os.path.join("usart", "usart_data", board_name, str(baud_rate))
        ...
        with ps2000.open_unit() as device:
            ...
            for message in messages:
                ...
                if isArduino:
                    arduino_code = arduino_code_template.format(len(message), message, baud_rate)
                    resp = upload_arduino_code(arduino_code, com_port, baud_rate, fqbn)

                    if resp == -1:
                        arduino_code = arduino_code_template.format((len(message)+1), message, baud_rate)
                        upload_arduino_code(arduino_code, com_port, baud_rate, fqbn)
