...
def upload_arduino_code(code, com_port, fqbn):
    try:
        ...
        os.makedirs(os.path.join("spi" ,temp_dir), exist_ok=True)

        sketch_file = os.path.join(os.path.join("spi" ,temp_dir), f"{temp_dir}.ino")
        with open(sketch_file, "w") as file:
            file.write(code)

        cmd = [
            "arduino-cli", "compile", "--fqbn", fqbn,
            "--port", com_port, os.path.join("spi" ,temp_dir)
        ]
        try:
            ...
            if err.returncode == 1:
                ...
                shutil.rmtree(os.path.join("spi" ,temp_dir))
                return -1
...
def spi_record_data(isArduino=False, fqbn="arduino:avr:mega"):
    ...
    folder_name = os.path.join("spi", "spi_data", board_name)
